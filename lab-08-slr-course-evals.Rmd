...
evals %>%
  summarise(mean = mean(score), median = median(score), min = min(score), max = max(score), standard_deviation = sd(score), q1 = quantile(score, probs = 0.25), q3 = quantile(score, probs= 0.75))
```

***The distribution is skewed. Looking at the histogram, we can see that the bulk of the scores are between 4 and 5, with very few between 0 and 3. From this we can determine that most students rate their professors highly.***
...
evals %>%
  ...
  labs(x = "Beauty Average", y = "Professor Evaluation Average", title = "Average Beauty Rating to Average Evaluation Score", subtitle = "Darker points means higher frequency")

```

***There is a positive relationship between Beauty Average and Evaluation Score***
...
***The initial graph stacked a bunch of the same values on top of each other so it was difficult to truly see how many points were on a certain value. The jitter plot adds some minor spacing to the points so that you can easily see how many points are on the same/similar values***
...
***The linear model is score = 3.88 + (0.07 multiplied by Beauty Rating)***
...
evals %>%
  ggplot(aes(x = bty_avg, y = score)) +
  geom_point(alpha = 0.4) +
  geom_smooth(method = "lm", se = FALSE, color = "#E35335") +
  labs(x = "Beauty Average", y = "Professor Evaluation Average", title = "Average Beauty Rating to Average Evaluation Score", subtitle = "Darker points means higher frequency")
```

6.  Interpret the slope of the linear model in context of the data.

***For an average beauty rating increased by 1, the average evaluation rating increases by 0.08***
...
***For an average beauty rating of 0, the average evaluation rating is 3.88. This does not make much sense because there are no beauty ratings with a value of 0 in the data***
...
***The linear model is score = 4.09 + (0.14 multiplied by Male)***
***A male professor scores 0.14 higher than a female one. Female professors on average score 4.09 on the ratings***

10. What is the equation of the line corresponding to male professors?
    What is it for female professors?
    
***Males: score = 4.09 + 0.14 = 4.23***
...
$$score = 4.28 - 0.13\times TenureTrack - 0.15\times Tenured$$
...
***The linear model is:***
$$score = 4.14 + 0.15\times Teaching + 0.02\times TenureTrack$$

***The average rating for a tenured professor is 4.14. The average rating for a teaching professor is 0.15 points higher than a tenured professor. The average for a tenure track professor is 0.02 points higher than a tenured professor.***
